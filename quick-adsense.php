...
/* 
	...
	Description: 一个不错的广告位管理插件，你可以很方便的添加删除管理您的博客广告，<a href="http://weibo.com/joychaocc">Joychao</a>汉化<br /> From <a hreef="http://wordpress.org/plugins/quick-adsense/">Quick Adsense</a>
	Author: Joychao(Chinesized)
	Version: 2.1.7
...
function process_content($content)
{
	...
	if( !$offdef ) {
		...
		if( $midd1 ) {
			if( mb_substr_count(strtolower($content), '</p>')>=2 ) {
				$sch = "</p>";
				$content = str_replace("</P>", $sch, $content);
				$arr = explode($sch, $content);			
				$nn = 0; $mm = mb_strlen($content)/2;
				for($i=0;$i<count($arr);$i++) {
					$nn += mb_strlen($arr[$i]) + 4;
					if($nn>$mm) {
						if( ($mm - ($nn - mb_strlen($arr[$i]))) > ($nn - $mm) && $i+1<count($arr) ) {
							$arr[$i+1] = '<!--'.$m1.'-->'.$arr[$i+1];							
						} else {
							$arr[$i] = '<!--'.$m1.'-->'.$arr[$i];
						}
	...
	if( strpos($content, '<!--'.$cusrnd.'-->') && ($showall || !$ismany) ) {
		$tcx = count($AdsId);
		$tcy = mb_substr_count($content, '<!--'.$cusrnd.'-->');
		for( $i=$tcx; $i<=$tcy-1; $i++ ) {
			array_push($AdsId, -1);
		}
		shuffle($AdsId);
	...
	if( strpos($content, '<!--RndAds-->') && ($showall || !$ismany) ) {
		...
		$tcx = count($AdsIdTmp);
		$tcy = mb_substr_count($content, '<!--RndAds-->');
 		for( $i=$tcx; $i<=$tcy-1; $i++ ) {
